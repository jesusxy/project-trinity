...
- **Loupe:** an oversized address expression leads into an exact source excerpt. The build extracts the `EntryPointVA` assignment from the pinned Loupe module's `inspect/inspect.go` into ignored `data/loupe_core.json`; Hugo renders that generated excerpt, and the build fails if it needs review. The shared-core architecture and Lab link remain.
